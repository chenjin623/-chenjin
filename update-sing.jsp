...
<link href="back_css/userIn2.css" rel="stylesheet" type="text/css" />
            ...
            <div class="settings-wrapper" id="pad-wrapper">               
				<div class="span3 avatar-box">
                    <div class="personal-image">
                        <img src="singer/<s:property value="#attr.sing.singImg"/>" class="avatar img-circle" />
                    </div>
                </div>
       <div class="info" style="position:relative;top:-130px;">
    	<h5 class="personal-title" style="font-size:26px; font-family:'华文行楷';position:absolute;top:110px;left:80px">修改歌手</h5>
        <div>
        	<form action="updateSinger" method="post" style="margin-left:177px;"  enctype="multipart/form-data"> 
        		<input class="text" name="singer.singerId" type="hidden" value="<s:property value="#attr.sing.singerId"/> "/>      
                <label>歌手姓名：</label>
                <input class="text" name="singer.artist" type="text" value="<s:property value="#attr.sing.artist"/> "/><br/>
                
                <label>性别：</label>
        			<input class="sex1" name="singer.singerSex" type="radio" 
        				<s:if test="#attr.sing.singerSex=='男'.toString()"> checked </s:if> value="男"/>
        			<p class="man">男</p>
        			
        			<input class="sex2" name="singer.singerSex" type="radio" 
        				<s:if test="#attr.sing.singerSex=='女'.toString()"> checked </s:if> value="女"/>
        			<p class="woman">女</p><br/>
        			
                    <input class="sex3" name="singer.singerSex" type="radio" 
        				<s:if test="#attr.sing.singerSex=='乐队'.toString()"> checked </s:if> value="组合"/>
                    <p class="ser">组合</p><br/>
                <label class="bir">出生日期：</label>
                	<div>
                	<%-- <s:date value="#attr.sing.singerbir" format="yyyy-MM-dd"/> --%>
                      <input name="singer.singerbir" class="jcDate" value="<s:date name="#attr.sing.singerbir" format="yyyy-MM-dd"/>"/>
                    </div>
                    <br/>
...
<script src="cityCSSandJS/citySet.js"></script>  
                ...
                <label class="area">所在地：</label>
                	<div style="position:absolute; left: 80px;top:185px;">
						<input name="singer.area" id="city" type="text"  value="<s:property value="#attr.sing.area"/>"  ov="请选择/输入城市名称"/>
					</div><br/>
...
//                 img.style.marginLeft = rect.left+'px';
        ...
        function clacImgZoomParam( maxWidth, maxHeight, width, height ){
            ...
            return param;
                ...
                <input name="upload"  class="text" type="file" style="width: 220px;" onchange="previewImage(this)"/><br/>  
                
                
                
                    
                <%-- <label class="song">曲风</label>
                <input class="text" name="singer.style" type="text" value="<s:property value="#attr.sing.style"/>" /><br/>  --%>
               <div class="field-box">
                   <label>曲风:</label>
                   <div class="ui-select" style="position: absolute;left:77px;top:455px; width: 220px; height: 30px;">
                     <select name="singer.style" id="user_time">
                     	   <option value="">请选择曲风</option>
                           <option value="流行" <s:if test="#attr.sing.style=='流行'.toString()">selected="selected"</s:if>>流行</option>
                           <option value="摇滚" <s:if test="#attr.sing.style=='摇滚'.toString()">selected="selected"</s:if>>摇滚</option>
                           <option value="电音" <s:if test="#attr.sing.style=='电音'.toString()">selected="selected"</s:if>>电音</option>
                           <option value="说唱" <s:if test="#attr.sing.style=='说唱'.toString()">selected="selected"</s:if>>说唱</option>
                           <option value="经典" <s:if test="#attr.sing.style=='经典'.toString()">selected="selected"</s:if>>经典</option>
                           <option value="爵士" <s:if test="#attr.sing.style=='爵士'.toString()">selected="selected"</s:if>>爵士</option>
                           <option value="民谣" <s:if test="#attr.sing.style=='民谣'.toString()">selected="selected"</s:if>>民谣</option>
                           <option value="乡村" <s:if test="#attr.sing.style=='乡村'.toString()">selected="selected"</s:if>>乡村</option>
                    </select> 
                   </div>
               </div>
               <label>歌手简介:</label>
               <textarea  class="from-control" class="text" name="singer.singIntroduction" style="width:380px;height:70px; pading-left:-10px">&nbsp;&nbsp;<s:property value="#attr.sing.singIntroduction"/>
               </textarea><br/>
               <input name=type type="hidden" value="singer"/>              
               <div class="span11 field-box actions" style="margin-top: 20px; margin-left: 90px;">
