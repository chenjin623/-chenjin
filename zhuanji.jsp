...
<!--  -->
	...
	<strong style="font-size:2em;" >全部专辑</strong>
	<h4>>>专辑>>第1页>></h4>		
	<div class="row"><div class="col-md-1">&nbsp;</div></div>	
	<div class="row">
	<s:iterator value="#attr.object" var="cd" status="st">
		<s:if test="(#st.getCount()%4)+1!=0">
		<div class="col-md-4">
        <div class="css5gallery">
        	<a class="list-block demo-2" href="getCdInfo?cd.cdId=<s:property value="cdId"/>">
        	<figure>
				<img src="cd/<s:property value="cdImg"/>" alt="fotograph" title="" style="width: 200px; height: 200px;" />
				<figcaption>
        			<h2><s:property value="cdName"/></h2>
        			<p>艺术家：<s:property value="singer.artist"/></p>
        			<p><s:date name="releaseTime" format="yyyy-MM-dd"/></p>
      			</figcaption>
			</figure>
			</a>
		</div>		
	...
	<nav class="text-center">
  			...
  			<s:if test="pageIndex>1">
    			<li >
    				<a href="fPageCD?type=cd&pageIndex=<s:property value="1"/>" aria-label="Previous"><span aria-hidden="true">首页</span></a>
    			</li>
  			</s:if>
  			...
  			<s:iterator begin="begin" end="end" status="st" var="i">
  				<li <s:if test="pageIndex==#i">class="active"</s:if>>
  					<a href="fPageCD?type=cd&pageIndex=<s:property value="#i"/>"><s:property value="#i"/> <span class="sr-only">(current)</span></a>
  				</li>
  			</s:iterator>
  			<s:if test="pageIndex<totalPage">
  				<li>
  					<a href="fPageCD?type=cd&pageIndex=<s:property value="totalPage"/>" aria-label="Previous"><span aria-hidden="true">尾页</span></a>
  				</li>
  			</s:if>
             </ul>
		</nav>
