...
<!--  -->
	...
	<form class="form-horizontal" onSubmit="return check();" action="reg" method="post">
  			...
  			<div class="form-group">
    			...
    			<div class="col-sm-4">
      			<div class="input-group input-group-lg">
 				<span class="input-group-addon" id="basic-addon1"><span style="color:#0099FF" class="glyphicon glyphicon-user"></span></span>
  				<input  onblur="chkUserId()" id="userId" name="user.userId" type="text" class="form-control" placeholder="请输入用户名" aria-describedby="basic-addon1">
				</div>
    			</div>
  			</div>
  			...
  			<div class="form-group">
    			...
    			<div class="col-sm-4">
      			<div class="input-group input-group-lg">
 				<span class="input-group-addon" id="basic-addon1"><span style="color:#0099FF" class="glyphicon glyphicon-lock"></span></span>
  				<input  name="user.userpassword" id="passwd"  id="passwd" type="password" class="form-control" placeholder="请输入密码" aria-describedby="basic-addon1">
				</div> 
    			</div>
  			</div>
  			...
  			<div class="form-group">
    			...
    			<div class="col-sm-4">
      			<div class="input-group input-group-lg">
 				<span class="input-group-addon" id="basic-addon1"><span style="color:#0099FF" class="glyphicon glyphicon-lock"></span></span>
  				<input  onblur="chkRePassword()" name="repassword" id="passwd2"  type="password" class="form-control" placeholder="请再次输入密码" aria-describedby="basic-addon1">
				</div> 
    			</div>
  			</div>
    		...
    		<div class="col-sm-offset-2 col-sm-7">
      			<button  type="submit" class="btn btn-lg btn-success" id="reg">提交注册</button>
    		</div>
  			</div>
  	</form>
    ...
    var chkUserIdRes=false;
    var chkPasswordRes=false;
    var chkRePasswordRes=false;
	//验证用户名
	function chkUserId(){
		var obj = document.getElementById("userId");
		/* var reg = /^[a-zA-Z]\w{3,19}$/; //字母开头的+3-19位字符
		 if(!reg.test(obj.value)){
			alert("帐号长度为4-20位，以字母开头，可写字母、数字、下划线！");
		}else if(obj.value!=null){
			alert("用户名不能为空！");
		} else{ */
			chkUserIdRes = checkUserName();
		/* } */
	}
	//验证密码
	function chkPassword(){
		var obj = document.getElementById("passwd");
		 var reg = /^\w{6,20}$/; //6-20位字符 
		 if(!reg.test(obj.value)){
			alert("密码长度6~20位，数字、字母、字符至少包含两种！");
		}else   if(obj.value!=null){
			alert("密码不能为空！");
		}else 
			chkPasswordRes = true;
	...
	function chkRePassword(){
		var obj = document.getElementById("passwd2");
		var objre = document.getElementById("passwd");
		 if(obj.value !== objre.value){
			alert("确认密码 ,密码与确认密码必须保持一致！");
		}else  if(obj.value!=null){
			alert("重复密码不能为空！");
		}else 
			chkRePasswordRes =  true;
	}
	//验证密保答案
	function chkAnswer(){
		var obj = document.getElementById("mibao");
    	 if(obj.value!=null){
			alert("密保答案不能为空！");
		}else 
			chkPasswordRes = true;
	...
	function chkForm(){
		return chkUserIdRes && chkPasswordRes && chkRePasswordRes;
	}
    </script>
    <script type="text/javascript">      
	...
	<script type="text/javascript" src="back_js/ajax.js"></script>
